...
public class FileTest {
	...
	public void setUpFixture(){
		...
		fileStringIntBoolean = new File("bestand.txt",100, true, Type.TXT);
		fileString = new File("bestand.txt",Type.TXT);
		timeAfterConstruction = new Date();

		timeBeforeConstructionNotWritable = new Date();
		fileNotWritable = new File("bestand.txt",100,false, Type.TXT);
	...
	public void testFileStringIntBoolean_IllegalCase() {
		timeBeforeConstruction = new Date();
		fileStringIntBoolean = new File("$IllegalName$",File.getMaximumSize(),false,Type.PDF);
		timeAfterConstruction = new Date();
	...
	public void testFileString_IllegalCase() {
		timeBeforeConstruction = new Date();
		fileString = new File("$IllegalName$",Type.TXT);
		timeAfterConstruction = new Date();
	...
	public void testEnlarge_LegalCase() {
		File file = new File("bestand.txt",File.getMaximumSize()-1,true,Type.TXT);
	...
	public void testHasOverlappingUsePeriod_ModifiedOverlap_D() {
		...
		File one, other;
		other = new File("one",Type.TXT);
		sleep(); // sleep() to be sure that one.getCreationTime() != other.getCreationTime()
		one = new File("other",Type.TXT);
